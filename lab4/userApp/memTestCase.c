...
int maxKallocSizeNow(int argc, unsigned char **argv){
	int i=0x1000,A;
	unsigned long x;
...
// for kernel test
int ktestCase1(int argc, unsigned char **argv){  
	//======for kalloc===============================
	char*buf1 = (char*)kalloc(19);
	char*buf2 = (char*)kalloc(24);
...
int ktestCase2(int argc, unsigned char **argv){
	//======for kalloc===============================
	char*buf1 = (char*)kalloc(11);
	char*buf2 = (char*)kalloc(21);
...
int ktestdP1(int argc, unsigned char **argv){
	unsigned long x,x1,xHandler;
	int i, tsize = 0x100;
	x = kalloc(tsize); 
	if (x){
		...
		kfree(x);
	} else myPrintf(0x7,"kalloc FAILED, CAN't TEST dPartition\n");	
}

int ktestdP2(int argc, unsigned char **argv){
	unsigned long x,x1,x2,x3,xHandler;
	int i, tsize = 0x100;
	x = kalloc(tsize); 
	if (x){
...
int ktesteFP(int argc, unsigned char **argv){
	unsigned long i=0x1000;//int
	unsigned long x,x1,x2,x3,x4,x5,xHandler;
	unsigned long psize = 31;//int
	unsigned long n = 4;//int
	...
	tsize = eFPartitionTotalSize(psize,n);
	x = kalloc(tsize); myPrintf(0x7,"X:0x%x:%d \n",x,tsize);
	if (x) {
		myPrintf(0x7, "We had successfully ");
		myPrintf(0x5, "kalloc()");
		myPrintf(0x7, " a small memBlock (size=0x%x, addr=0x%x);\n", tsize,x);
		...
		eFPartitionFree(xHandler,x4);		
		eFPartitionWalkByAddr(xHandler);
	} else myPrintf(0x7,"TSK2: kalloc FAILED, CAN't TEST eFPartition\n");

	return 0;
}

int ktestdP3(int argc, unsigned char **argv){
	unsigned long x,x1,x2,x3,xHandler;
	int i, tsize = 0x100;
	x = kalloc(tsize); 
	if (x){
...
void memTestCaseInit(void){	
	...
	// for kalloc
	addNewCmd("maxkallocSizeNow\0", maxKallocSizeNow, NULL, "MAX_KALLOC_SIZE always changes. What's the value Now?\0");
	addNewCmd("ktestMalloc1\0", ktestCase1, NULL, "kalloc, write and read.\0");
	addNewCmd("ktestMalloc2\0", ktestCase2, NULL, "kalloc, write and read.\0");		

	addNewCmd("ktestdP1\0", ktestdP1, NULL, "Init a dPatition(size=0x100) [Alloc,Free]* with step = 0x20\0");
	addNewCmd("ktestdP2\0", ktestdP2, NULL, "Init a dPatition(size=0x100) A:B:C:- ==> -:B:C:- ==> -:C:- ==> - .\0");
	addNewCmd("ktestdP3\0", ktestdP3, NULL, "Init a dPatition(size=0x100) A:B:C:- ==> A:B:- ==> A:- ==> - .\0");
	
	addNewCmd("ktesteFP\0", ktesteFP, NULL, "Init a eFPatition. Alloc all and Free all.\0");
}
